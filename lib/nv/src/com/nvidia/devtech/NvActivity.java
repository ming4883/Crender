    ...
    implements SensorEventListener
{
    ...
    /**
     * Function called when the java side wants to retrieve a human-readable
     * error message from the native side application code.
     * 
     * Not abstract so that subclasses don't HAVE to implement messages.
     * 
     ...
     * @return True if the event was handled.
     */
    public abstract boolean keyEvent(int action, int unicodeChar, KeyEvent event);
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event)
    {
        ...
        if (!ret)
            ret = keyEvent(event.getAction(), event.getUnicodeChar(), event);
        return ret;
    }
