...
{
	CrContextImpl* self = (CrContextImpl*)crMem()->alloc(sizeof(CrContextImpl), "CrContext");
...
CR_API CrBool crContextInit(CrContext* self, void** window)
{
	...
	self->xres = NVEventEGLGetSurfaceWidth();
	self->yres = NVEventEGLGetSurfaceHeight();
	// create FBOs
...
CR_API void crContextFree(CrContext* self)
{
	...
	current = nullptr;
	if(0 != impl->rttFBOName) {
		glDeleteFramebuffers(1, &impl->rttFBOName);
	}
